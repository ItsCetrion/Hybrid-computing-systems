...
class WarpShuffleReductionSumTest : public ::testing::TestWithParam<std::tuple<std::size_t, float>> {
    protected:
    void matsum_test_impl(std::size_t size, float tol) {
        ...
        float my_vector_result = sum(my_vector, WarpShuffleTag{});
        EXPECT_NEAR(en_vector_result, my_vector_result, tol);
    }
};

TEST_P(WarpShuffleReductionSumTest, matsum_test) {
  auto [size, tol] = GetParam();
  matsum_test_impl(size, tol);
}

INSTANTIATE_TEST_SUITE_P(
    WarpShuffleReductionSumTestSuite,
    WarpShuffleReductionSumTest,
    ::testing::Values(
